...
In previous lectures we've seen problems being solved by
%Lecture~39, we designed an algorithm to solve the maximum matching problem on bipartite graphs
\emph{transforming} them into a maximum-flow problem. These algorithms call a
solver for the maximum-flow problem~(in step 2), and also build the network~(in step 1)
and return the solution by processing the maximum-flow or minimum-cut~(in step 3). 
These are examples of so-called \emph{polynomial-time reduction}.
...
Problem $X$ is polynomial-time reducible to $Y$ means ``we can design an algorithm for $X$ using an algorithm for $Y$''.
...
Applications of maximum-flow are such examples, where the number of calls
to a solver for max-flow problem is 1; there are extra computational steps 
but they are polynomial. So, these problems are polynomial-time reducible to the max-flow problem.
We can formally write as, for example,
%in that algorithm, we called once a solver for the maximum-flow problem; and the steps 1 and 3
%takes polynomial-time. Therefore, we have 
(maximum-matching-problem-on-bipartite-graph) $\le_p$ (maximum-flow-problem).
...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {5}{algorithm-for-$VC(G, k)$}\xxx
...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {12}{algorithm-for-$VC(G)$}\xxx
	\aab {$k^* \leftarrow $ algorithm-for-cardinality-$VC(G)$;}\xxx
	\aaB {7}{for each $v \in V$}\xxx
...
and such certificate must exist for ``true''-instances and must not exist for ``false''-instances.
...
Let $X$ be a decision problem. We say $X\in \textrm{P}$ if there exists a polynomial-time algorithm $A$~(we call $A$ as a \emph{solver}), such that,\\
	...
	\hspace*{0.2cm} if $x$ is a ``true''-instance, then $A(x)$ returns ``yes'';\\
	\hspace*{0.2cm} if $x$ is a ``false''-instance, then $A(x)$ returns ``no''.
\end{definition}
...
Let $X$ be a decision problem. We say $X\in \textrm{NP}$ if there exists a polynomial-time algorithm $A$~(we call $A$ as a \emph{verifier}), such that,\\
	...
	(2), for every instance $x$ of problem $X$: \\
	\hspace*{0.2cm} if $x$ is a ``true''-instance, then there must exist a certificate $c$ such that $A(x,c)$ returns ``yes'';\\
	\hspace*{0.2cm} if $x$ is a ``false''-instance, then for any polynomial-length certificate $c$, $A(x,c)$ always returns ``no''.
\end{definition}
...
for every ``true''-instnace but not for any ``false''-instance, and they can be correctly verified by algorithm $A$.
...
such certificate \emph{exists}, we do not need to design algorithms to find it.
...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {7}{verifier~$(x = (G, k), c)$}\xxx
	\aab {verify $c$ is a subset of vertices of $G$: if not, return ``no'';}\xxx
...
If $x = (G, k)$ is an ``true''-instance, then it means there exists a subset of vertices $V_1$
...
If $x$ is an ``false''-instance, then there does not exist a subset with at most $k$ vertices
that covers all edges in $G$.
Hence, no matter what is coded in $c$ and is passed to the verifier with $x$,
the verifier always returns ``no''---either because it is not a subset of vertices,
or its size exceeds $k$,
or it does not cover all edges.  Also, verifier $A$ runs in polynomial-time.
\qed
