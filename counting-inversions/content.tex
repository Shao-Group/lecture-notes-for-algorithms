...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {6}{Algorithm~\#1: brute-force-count-inv~($A[1\cdots n]$)}\xxx
	\aab {let inv = 0;}\xxx
	\aaB {3}{for $i = 1$ to $n$}\xxx
...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {6}{function count-inv-two-arrays~($S[1\cdots m], T[1\cdots n]$)}\xxx
	\aab {let inv = 0;}\xxx
	\aaB {3}{for $i = 1$ to $m$}\xxx
...
\begin{algorithmblock}[0.8\textwidth]
        \aaA {15}{function merge-count-inv-two-sorted-arrays ($S[1\cdots m], T[1\cdots n]$)}\xxx
        ...
        \aaB {8}{for $k = 1 \to m + n$}\xxx
        \faC {3}{if $S[i] \le T[j]$}\xxx
        \aad {$C[k] = S[i]$;}\xxx
        \aad {$i = i + 1$;}\xxx
        \aaC {4}{else}\xxx
        \aad {$C[k] = T[j]$;}\xxx
