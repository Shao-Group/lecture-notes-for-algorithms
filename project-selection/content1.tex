...
The profit could be positive or negative (otherwise the problem becomes trivial, as you can tell later).
...
of the constructed network, and we will also show that maximizing the
...
connect each project with non-negative profit to sink $t$. Formally, edges in the network is: 
...
We have 
$$c(S, T) = \sum_{v\in T_-} -p(v) + \sum_{v\in S_+} p(v).$$
The total profit of $V_1$ is 
$$\sum_{v\in V_1} p(v) = \sum_{v\in T_-} p(v) + \sum_{v\in T_+} p(v).$$
We sum up both sides. Note that term $\sum_{v\in T_-} p(v)$ cancels out.
We have 
$$c(S, T) + \sum_{v\in V_1} p(v) = \sum_{v\in S_+} p(v) + \sum_{v\in T_+} p(v).$$
Note that the right side is exactly the total profit of projects with non-negative profit, i.e., $\sum_{v\in V: p(v) \ge 0} p(v)$,
...
have the maximized total profit. This is summarized below, which concludes the proof.
\begin{fact}
...
the preflow-push algorithm. Hence, the algorithm for project selection problem can be solved in polynomial-time as well.
