...
\subsection*{Maximum-Matching vs.\ Minimum-Vertex Cover}
...
See also in Figure~\ref{fig:matching} for an example.
...
Is it true that for \emph{every} undirected graph there always exists a matching $M$ and a 
vertex cover $V_1$ that satisfies $|M| = |V_1|$?
The answer is no; the Figure~(in the previous Lecture) gives such an example.
...
\subsection*{Maximum Matching and Minimum Vertex Cover of Bipartite Graphs}
...
we have $|V_1| = |X\cap T^*| + |Y\cap S^*| = c(S^*, T^*) = |f^*| = |M|$. 
This gives that the vertex cover $V_1$ has the same size with the matching $M$,
	 proving that both are optimal. 
...
exist polynomial-time (optimal) algorithm for it unless P{} = {}NP.
But the maximum matching problem (on general undirected graphs) can be solved in polynomial-time,
 with the Blossom algorithm~(Edmonds, 1961).
\end{enumerate}
