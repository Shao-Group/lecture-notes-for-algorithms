...
\caption{Augmenting an $s$-$t$ path in the residual graph. 
Suppose in the residual graph $G_f$ the algorithm identifies $s$-$t$
...
condition remains satisfied owes to how the residual graph and its edge capacities are constructed.
...
$(S^*, T^*)$ is an $s$-$t$ cut. See Figure~\ref{fig:theorem}.
...
Right: the same $s$-$t$ cut $(S^*, T^*)$ drawn on the (original) network $G$, where one can see that for every edge $e\in E(S^*, T^*)$, $f^*(e) = c(e)$,
and for every edge $e\in E(T^*, S^*)$, $f^*(e) = 0$.}
\label{fig:theorem}
\end{figure}


We now show the central result: $|f^*| = c(S^*,T^*)$. Recall that
the value of any flow, here we focus on $f^*$, can be calculated with any $s$-$t$
...
$(v, u)$ because $f^*(e) > 0$. Consequently, $s$ can reach $u$ in $G_{f^*}$, as $s$ can reach $v$~(because $v\in S^*$) and $v$ can reach $u$ following this backward edge. 
...
\subsection*{Faster Algorithms for the Maximum Flow Problem}

Max-flow problem is fundamental and instrumental for computer science.
Although the Ford-Fulkson algorithm does not run in polynomial-time~(it is in fact a pseudo-polynomial-time algorithm),
there are (pure) polynomial-time algorithms for it.
It remains an active research area to design faster algorithms.
Below is a summary of major algorithms; note that the latest algorithm,
...
\begin{tabular*}{0.9\textwidth}{@{\extracolsep{\fill}}lcl}
\hline
\textbf{Algorithm} & \textbf{Year} & \textbf{Time Complexity} \\ \hline
Ford-Fulkerson & 1956 & $O\!\left(|E|\,|f^*|\right)$ \\ \hline
Edmonds-Karp & 1972 & $O\!\left(|V|\,|E|^2\right)$ \\ \hline
%Dinic's algorithm & 1970 & $O\!\left(|V|^2|E|\right)$ \\ \hline
Karzanov's algorithm & 1974 & $O\!\left(|V|^3\right)$ \\ \hline
%MPM algorithm & 1978 & $O\!\left(|V|^3\right)$ \\ \hline
%Push--relabel & 1986 & $O\!\left(|V|^2|E|\right)$ \\ \hline
%Goldberg--Rao & 1998 & $O\!\left(\min\!\left(|V|^{2/3},|E|^{1/2}\right)\,|E|\, \log\!\left(\frac{|V|^2}{|E|}\right)\log U\right)$ \\ \hline
Orlin's algorithm & 2013 & $O\!\left(|V|\,|E|\right)$ \\ \hline
Almost-linear-time algorithm & 2022 & $O(|E|^{1+o(1)})$ \\ \hline
\end{tabular*}
\caption{Major algorithms for the maximum-flow problem. }
\label{tab:maxflow-algorithms}
\end{table}
