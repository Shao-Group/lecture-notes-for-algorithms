...
Such self-pairing structure is called the secondary structure of RNA.
...
The inference of RNA secondary structure is a \emph{science} question. More factors
...
is a \emph{model} of RNA secondary structure inference. It's not perfect. In fact, ``all models are wrong''.
...
We again design a dynamic programming algorithm.
...
It also suggests defining subproblems \emph{ove rall substrings}~(or over intervals) of $X$.
...
\begin{figure}[b!]
\centering{\input{rna-recurrence}}
\caption{Two cases in developing the recurrence.}
\label{fig:rna-recurrence}
\end{figure}


We now develop a recurrence, which again is the reverse process of reasoning the optimal substructure
but with necessary enumerations.
See Figure~\ref{fig:rna-recurrence}.
...
$$P(i,j) = \max_{i \le k < j, X[j]\sim X[k] } (1 + P(i, k-1) + P(k+1,j-1)),$$
where the extra 1 represents the pair of $(X[k], X[j])$ and I use $X[j]\sim X[k]$ to represent that $X[j]$ and $X[k]$ can be paired.

Combined, we have the following recurrence.
\begin{displaymath}
P(i,j) = \max\left\{
	\begin{array}{llll}
	P(i,j-1)\\
...
To faciliate applying the recurrence, we initialize $P(i, i - 1) = 0$, $1 \le i \le n$.
...
of each entry takes $\Theta(n)$ time. The space complexity of this algorithm is $\Theta(n^2)$.

\begin{algorithmblock}[0.8\textwidth]
	\aaA {11}{Algorithm rna-secondary-structure~(string $X$ of length $n$)}\xxx
	\aab {$P(i,i-1) = 0$, for any $1 \le i \le n$}\xxx
	\aaB {8}{for $d = 1 \to n$}\xxx
	\aaC {7}{for $i = 1 \to n-d+1$}\xxx
	\aad {$j = i + d -1$;}\xxx
	\aad {$P(i,j) = P(i, j - 1)$;}\xxx
	\aaD {4}{for $k = i \to j - 1$}\xxx
	\aaE {3}{if~($X[k]$ and $X[j]$ can be paired, and $P(i,j) < P(i,k-1) + P(k+1,j-1) + 1$);}\xxx
