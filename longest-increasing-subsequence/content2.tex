...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {6}{Algorithm LIS-DP~(array $A[1\cdots n]$)}\xxx
	\aaB {4}{for $k = 1 \to n$}\xxx
	\aac {$L[k] = 1$;}\xxx
...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {5}{Algorithm LIS-traceback~($A[1\cdots n]$, $T[1\cdots n]$, $k$)}\xxx
	\aaB {3}{while~($k\neq nul$)}\xxx
...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {8}{Algorithm LIS-DP-T~(array $A[1\cdots n]$)}\xxx
	\aaB {6}{for $k = 1 \to n$}\xxx
	\aac {$L[k] = 1$;}\xxx
	\aac {$T[k] = nul$;}\xxx
	\aaC {3}{for $j = 1 \to k - 1$}\xxx
...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {16}{Algorithm faster-LIS-full~(array $A[1\cdots n]$)}\xxx
	\aab {$S=\emptyset$; }\xxx
	\aaB {9}{for $k = 1 \to n$}\xxx
	\faC {3}{if $|S| = 0$ or $A[k] > S[|S|]$}\xxx
