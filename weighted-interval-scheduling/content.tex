...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {10}{function binary-search~($A[1\cdots n]$, $k$, $a$, $b$)}\xxx
	\aab {$m = (a + b) / 2$;}\xxx
	\faB {2}{if~($t(m) \le s(k) \le t(m + 1)$)\ \ // $pre[k] = m$}\xxx
	\aac {return $m$;}\xxx
	\faB {2}{else if~($t(m) > s(k)$)\ \ // $pre[k]$ must be in $[a\cdots m]$}\xxx
	\aac {return binary-search~($A$, $k$, $a$, $m$);}\xxx
	\faB {2}{else if~($s(k) > t(m + 1)$) \ \ // $pre[k]$ must be in $[m + 1\cdots b]$}\xxx
...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {7}{Algorithm DP-weighted~(intervals $A[1\cdots n]$, $v(i)$, $1\le i \le n$)}\xxx
