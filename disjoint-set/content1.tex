...
The collection of sets is stored using a \emph{forest}~(i.e., a collection of trees).
...
until reaching the root. It then returns the root as it is the representative
of the set. Note that the running time of this procedure
is propotional to the height of the tree.

\begin{algorithmblock}[0.8\textwidth]
	\aaA {4}{function find~($x$)}\xxx
	\aaB {2}{while~($x.parent \neq x$)}\xxx
	\aac {$x = x.parent$;}\xxx
	\aab {return $x$;}\xxx
\end{algorithmblock}

The union operation first finds the roots of the two trees that contains $x$ and
...
\begin{algorithmblock}[0.8\textwidth]
	...
	\faB {2}{if~($r_x.height < r_y.height$)}\xxx
	\aac {$r_x.parent = r_y$}\xxx
	\faB {2}{else if~($r_x.height > r_y.height$)}\xxx
