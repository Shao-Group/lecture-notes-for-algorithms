...
We will develop a key property that will be used to prove the correctness of both Kruskal's algorithm
and the Prim's algorithm. We start with introduce the definition of \emph{cut} and \emph{cut-edges}.
...
For MST problem the underlying graphs are undirected. (In the network algorithms, we will
use cut/cut-edges for directed graphs.)
...
are essentially using this property~(in different ways).
...
can be safely added to $E_1$.


See Figure~\ref{fig:spanning}. The blue edges~(including thin and thick ones)
...
We now prove Claim~\ref{claim:cut-property}.  See Figure~\ref{fig:proof}.
...
in each iteration, it picks the \emph{smallest} edge that connects to a new vertex.
First, edges that connect $S$ to a new vertex are \emph{exactly} the cut-edges of cut $(S, V\setminus S)$.
Hence, in each iteration, Prim's algorithm 
\emph{exactly} picks the smallest cut-edge of $(S,V\setminus S)$.
Second, we have $E(C) \cap E_1 = \emptyset$, as $E_1$ connects vertices in $S$. (Here is a formal proof by
contradiction: assume that $E(C) \cap E_1 \neq \emptyset$ and $e = (u,v) \in E_1\cap E(C)$; 
consider the time that $e$ is picked and added to $E_1$ and at that time according to the algorithm,
$u$ must be already included in $S$ and $v$ will be added to $S$ immediately; this is a contradition
to the fact that $e$ is a cut-edge.)
Now, apply the cut-property: adding the smallest cut-edge to $E_1$ results in a (larger) partial MST.
This is exactly what the Prim's algorithm does.
So, throughout the algorithm, $E_1$ remains a partial MST.
When $|E_1| = |V|-1$, $E_1$ becomes a MST.
...
until $E_1$ reaches $|V| - 1$ edges. At any time of the algorithm,
...
So, in each iteration, Kruskal's algorithm exactly picks the smallest edge among these that span all pairs of
...
Next, we have $E(C)\cap E_1 = \emptyset$, for the same reasoning that the two ends of every edge in $E_1$ must be in the same connected component~(and hence cannot be a cut-edge).
Hence, applying the cut-property here proves that $E_1\cup \{e^*\}$ is a partial MST.
This invariant again holds throughout the algorithm; when the algorithm terminates, i.e., $E_1$ becomes a spanning tree,
it must be also a minimum spanning tree.
