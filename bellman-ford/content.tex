...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {3}{procedure update(edge $(u,v)\in E$)}\xxx
...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {11}{Algorithm Bellman-Ford-Detect-Negative-Cycle~($G = (V, E)$, $l(e)$ for any $e\in E$, $s \in V$)}\xxx
	\aab {init an array $dist$ of size $|V|$;}\xxx
	\aab {$dist[s] = 0$; $dist[v] = \infty$ for any $v\neq s$;}\xxx
%	\aab {\textcolor{black}{$prev[v] = null$, for any $v\in V$};}\xxx
	\aaB {3}{for $k = 1 \to |V| - 1$}\xxx
	\aaC {2}{for each edge $(u,v)\in E$}\xxx
	\aad {$update(u,v)$;}\xxx
	\aaB {3}{for each edge $(u,v)\in E$}\xxx
...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {8}{Algorithm BF-DAG~(DAG $G = (V, E)$, $l(e)$ for any $e\in E$, $s \in V$)}\xxx
	\aab {init array $dist$ of size $|V|$: $dist[s] = 0$; $dist[v] = \infty$ for any $v\neq s$;}\xxx
	\aab {calculate a linearization of $G$;}\xxx
	\aaB {3}{\textcolor{blue}{for $v\in V$ following the order of linearization}}\xxx
