...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {4}{procedure update-dist~($v_{k+1}^*$)}\xxx
	\aaB {3}{for $(v_{k+1}^*, v) \in E$}\xxx
...
\begin{algorithmblock}[0.8\textwidth]
	\aaA {14}{Algorithm Dijkstra~($G = (V, E)$, $l(e)$ for any $e\in E$, $s \in V$)}\xxx
	...
	\aaB {7}{while~(empty~($PQ$) = false)}\xxx
	\aac {$u$ = find-min~($PQ$);}\xxx
	\aac {delete-min~($PQ$);}\xxx
	\aaC {4}{for each edge~$(u, v)\in E$}\xxx
